...
title Piloto Setup (Laravel) - PetFlow
...
echo.
echo Ejecutando: php artisan pilot:setup --fresh --demo
echo.

if /i "%PHP_EXE%"=="php" (
  php artisan pilot:setup --fresh --demo >> "%LOG_FILE%" 2>&1
) else (
  "%PHP_EXE%" artisan pilot:setup --fresh --demo >> "%LOG_FILE%" 2>&1
)
