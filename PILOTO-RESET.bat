...
title Piloto Reset (Laravel) - PetFlow

set "ROOT=%~dp0"
set "BACKEND=%ROOT%backend"
set "LOG_FILE=%ROOT%pilot-reset.log"

echo [%date% %time%] START > "%LOG_FILE%"

echo ==========================================================
echo RESET PILOTO (LIMPIO) - Borra TODA la BD y recrea lo minimo
echo Log: "%LOG_FILE%"
echo ==========================================================
echo.
echo ADVERTENCIA: Esto elimina TODOS los datos en DB (migrate:fresh)
echo.
...
echo.
echo Ejecutando: php artisan pilot:setup --fresh
echo.

if /i "%PHP_EXE%"=="php" (
  php artisan pilot:setup --fresh >> "%LOG_FILE%" 2>&1
) else (
  "%PHP_EXE%" artisan pilot:setup --fresh >> "%LOG_FILE%" 2>&1
)
